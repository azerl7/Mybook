package com.geo.mybook.distributed.id.generator.biz.core.snowflake;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.geo.mybook.distributed.id.generator.biz.core.common.PropertyFactory;
import com.geo.mybook.distributed.id.generator.biz.core.snowflake.exception.CheckLastTimeException;
...
public class SnowflakeZookeeperHolder {
    ...
    private int sessionTimeoutMs = 60000; // 与ZK服务端对齐：60秒
    private int connectionTimeoutMs = 15000; // 连接超时：15秒
    private RetryPolicy retryPolicy = new ExponentialBackoffRetry(1000, 3);//指数退避重试
    ...
    public boolean init() {
        try {
            System.out.println("---------  "+connectionString+"     "+listenAddress+"   "+connectionTimeoutMs+"     "+sessionTimeoutMs);
            CuratorFramework curator = createWithOptions(connectionString, retryPolicy, connectionTimeoutMs, sessionTimeoutMs);
            curator.start();
            // 新增：等待Curator连接稳定（避免会话未建立就发起操作）---------//azerl7//--------
            boolean connected = curator.blockUntilConnected(10, TimeUnit.SECONDS);
            if(!connected){
                throw new RuntimeException("Curator 连接 zk 超时，10秒内未建立连接");
            }
